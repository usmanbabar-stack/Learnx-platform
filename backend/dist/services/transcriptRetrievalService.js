...
async function retrieveRelevantChunks(transcript, query, opts = {}) {
    ...
    const useEmb = Boolean(String((opts.useEmbeddings ?? (process.env.USE_EMBEDDINGS || "false"))).toLowerCase() === "true");
    ...
    if (useQdrant && videoId) {
        try {
            const qdrantResults = await qdrantService_1.qdrantService.searchChunks(videoId, query, k * 2, 0.3);
            if (qdrantResults.length > 0) {
                logger_1.logger.info(`✅ Qdrant found ${qdrantResults.length} chunks for ${videoId}`);
                // Convert Qdrant chunks to TranscriptChunk format
                let topChunks = qdrantResults.map(r => r.chunk);
            ...
            logger_1.logger.warn("Qdrant search failed, falling back to in-memory search:", error);
        }
    }
    ...
    if (useEmb && !useQdrant) {
        try {
            ...
            const qEmb = await (0, geminiService_1.getEmbeddings)([query], model);
            const cEmb = await (0, geminiService_1.getEmbeddings)(chunks.map(c => c.text), model);
            ...
            logger_1.logger.warn("Embedding rerank failed, continuing with lexical only: %o", e);
        }
    }
    // Select top-k
...
//# sourceMappingURL=transcriptRetrievalService.js.map