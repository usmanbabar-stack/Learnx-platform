...
const TranscriptItemSchema = new mongoose_1.Schema({
    text: { type: String, required: true },
    start: { type: Number, required: true },
    duration: { type: Number, required: true }
}, { _id: false });
const VideoMetadataSchema = new mongoose_1.Schema({
...
VideoSchema.statics.findBySubject = function (subject, limit = 20) {
    return this.find({ subject, isEducational: true })
        .sort({ qualityScore: -1, createdAt: -1 })
        .limit(limit);
};
VideoSchema.statics.searchVideos = function (query, filters = {}, limit = 20) {
    const searchQuery = {
        $and: [
            { isEducational: true },
            {
    ...
    // Apply filters
    if (filters.subject)
        searchQuery.$and.push({ subject: filters.subject });
    if (filters.difficulty)
        searchQuery.$and.push({ difficulty: filters.difficulty });
    if (filters.language)
        searchQuery.$and.push({ language: filters.language });
    if (filters.minQualityScore)
        searchQuery.$and.push({ qualityScore: { $gte: filters.minQualityScore } });
