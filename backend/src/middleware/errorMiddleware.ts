...
export const errorHandler = (
  ...
  if (err.code === '23505') {
    ...
    logger.error('CRITICAL: Missing database table', { error: err.message });
  } else if (err.code === '28P01') {
    // Invalid password
    statusCode = 503;
    message = 'Database authentication failed.';
    logger.error('CRITICAL: Database auth failed');
  }
  ...
  if (err.message?.includes('ENOMEM') || err.message?.includes('out of memory')) {
    ...
    logger.error('CRITICAL: Out of memory error');
  }
...
export const asyncHandler = (fn: Function) => (req: Request, res: Response, next: NextFunction) => {
  Promise.resolve(fn(req, res, next)).catch((error) => {
    ...
    if (!(error instanceof Error)) {
      const wrappedError = new Error(String(error)) as CustomError;
