...
import { VideoDocument, VideoMetadata, TranscriptItem, SearchFilters } from '../types/video';
...
export class VideoRepository {
  ...
  async create(video: VideoDocument): Promise<VideoDocument> {
    return await this.pool.query('BEGIN').then(async () => {
      try {
        ...
        if (video.transcript && video.transcript.length > 0) {
          const transcriptValues = video.transcript.map((item, idx) => 
            `($1, $${idx * 3 + 2}, $${idx * 3 + 3}, $${idx * 3 + 4}, $${idx * 3 + 5})`
          ).join(', ');
          
          const transcriptParams: any[] = [videoId];
          video.transcript.forEach((item, idx) => {
            transcriptParams.push(item.text, item.start, item.duration, idx);
          });

          await this.pool.query(
            `INSERT INTO transcript_items (video_id, text, start_time, duration, sequence_order) VALUES ${transcriptValues}`,
            transcriptParams
          );
        }
        ...
        if (video.searchKeywords && video.searchKeywords.length > 0) {
          const keywordValues = video.searchKeywords.map((_, idx) => 
            `($1, $${idx + 2})`
          ).join(', ');
          
          const keywordParams: any[] = [videoId, ...video.searchKeywords];

          await this.pool.query(
            `INSERT INTO video_keywords (video_id, keyword) VALUES ${keywordValues} ON CONFLICT DO NOTHING`,
            keywordParams
          );
        }
  ...
  async findBySubject(subject: string, limit: number = 20): Promise<VideoDocument[]> {
    const query = `
      SELECT 
        v.*,
        COALESCE(
  ...
  async searchVideos(query: string, filters: SearchFilters = {}, limit: number = 20): Promise<VideoDocument[]> {
    const conditions: string[] = ['v.is_educational = true'];
    const params: any[] = [];
    let paramIndex = 1;
  ...
  async getTranscriptByVideoId(videoId: string): Promise<TranscriptItem[]> {
    const query = `
  ...
  async saveTranscript(videoId: string, segments: TranscriptItem[]): Promise<void> {
    ...
    try {
      ...
      logger.info(`Saved ${segments.length} transcript segments for video: ${videoId} (bulk insert)`);
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error(`Failed to save transcript for ${videoId}:`, error);
      throw error;
    } finally {
      client.release();
    }
  ...
  private mapRowToVideoDocument(row: any): VideoDocument {
    ...
    const transcript: TranscriptItem[] = Array.isArray(row.transcript) 
...
export async function saveSummary(summary: SavedSummary): Promise<void> {
  ...
  logger.info(`Saved summary to database for video: ${summary.videoId}`);
}

// Glossary Repository functions
export async function getGlossaryByVideoId(videoId: string): Promise<SavedGlossary | null> {
  const pool = getPostgresPool();
...
export async function saveGlossary(glossary: SavedGlossary): Promise<void> {
  const pool = getPostgresPool();
  const query = `
  ...
  logger.info(`Saved glossary to database for video: ${glossary.videoId}`);
}

// Delete glossary from database (for clearing failed generations)
export async function deleteGlossary(videoId: string): Promise<boolean> {
  const pool = getPostgresPool();
  const result = await pool.query(
    'DELETE FROM video_glossaries WHERE video_id = $1',
    [videoId]
  );
  
  if (result.rowCount && result.rowCount > 0) {
    logger.info(`Deleted glossary from database for video: ${videoId}`);
    return true;
  }
  return false;
}
...
export async function getChatHistory(userId: number, videoId: string): Promise<ChatHistoryMessage[]> {
  const pool = getPostgresPool();
  const query = `
...
export async function clearChatHistory(userId: number, videoId: string): Promise<number> {
  const pool = getPostgresPool();
  const result = await pool.query(
    'DELETE FROM chat_history WHERE user_id = $1 AND video_id = $2',
    [userId, videoId]
  );
  
  const deletedCount = result.rowCount || 0;
  logger.info(`Cleared ${deletedCount} chat messages for user ${userId}, video ${videoId}`);
  return deletedCount;
}
