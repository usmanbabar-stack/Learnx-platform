...
export class AnalyticsController {
  ...
  async getOverview(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [
        ...
        Video.countDocuments({ isEducational: true, ...matchStage }),
        
        Video.distinct('subject', { isEducational: true, ...matchStage }).then(subjects => subjects.length),
        ...
        Video.find({ isEducational: true, ...matchStage })
          .sort({ createdAt: -1 })
          .limit(10)
  ...
  async getPopularVideos(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('Error in getPopularVideos:', error);
      res.status(500).json({
        success: false,
        message: 'Internal server error'
      });
  ...
  async getSearchTrends(req: Request, res: Response): Promise<void> {
    try {
      ...
      const trendingKeywords = await Video.aggregate([
        { $match: { isEducational: true, ...matchStage } },
      ...
      const trendingSubjects = await Video.aggregate([
        { $match: { isEducational: true, ...matchStage } },
        {
          $group: {
            _id: '$subject',
   ...
   */
  async getSubjectAnalytics(req: Request, res: Response): Promise<void> {
    try {
      const { startDate, endDate } = req.query;
      ...
      const subjectAnalytics = await Video.aggregate([
        { $match: { isEducational: true, ...matchStage } },
        {
          $group: {
            _id: '$subject',
      ...
      logger.error('Error in getSubjectAnalytics:', error);
      res.status(500).json({
        success: false,
        message: 'Internal server error'
      });
  ...
  async getVideoAnalytics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const transcriptStats = {
        ...
        totalWords: video.transcript.reduce((count, segment) => 
      ...
      logger.error('Error in getVideoAnalytics:', error);
      res.status(500).json({
        success: false,
        message: 'Internal server error'
      });
