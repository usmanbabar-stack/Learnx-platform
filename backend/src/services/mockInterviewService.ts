...
// Using gemini-pro (stable model with high quota)
...
export class MockInterviewService {
  ...
  private cache: Map<string, MockInterview> = new Map();
  ...
  async generateInterview(
    ...
    if (this.cache.has(cacheKey)) {
      logger.info(`Cache hit for mock interview: ${cacheKey}`);
      return this.cache.get(cacheKey)!;
    }

    logger.info(`Generating mock interview for field: ${field}, difficulty: ${difficulty}, questions: ${questionCount}`);
...
Make questions realistic, professional, and tailored to ${field}. Ensure variety in question types and difficulty progression.`;
    ...
    try {
      ...
      logger.info(`Gemini raw response length for interview: ${text?.length || 0}`);

      if (!text || text.trim().length === 0) {
        logger.warn(`Empty response for mock interview`);
      ...
      try {
        ...
        logger.error('Interview JSON parse failed:', jsonStr.slice(0, 500));
        return this.getFallbackInterview(field, difficulty, questionCount);
      }

      const questions: InterviewQuestion[] = (parsed.questions || []).map((q: any, index: number) => ({
        id: String(index + 1),
      ...
      logger.info(`✅ Mock interview generated: ${questions.length} questions for ${field}`);

      return interview;
    } catch (error: any) {
      logger.error('Mock interview generation failed:', {
  ...
  async evaluateAnswer(
    ...
    logger.info(`Evaluating answer for field: ${field}`);
...
Be constructive, specific, and encouraging in your feedback.`;
    ...
    try {
      ...
      logger.info(`✅ Answer evaluated with score: ${feedback.score}`);
      return feedback;
    } catch (error: any) {
      logger.error('Answer evaluation failed:', error);
      return this.getFallbackFeedback(userAnswer, question, expectedKeyPoints, field);
    }
  }
  ...
  private getFallbackInterview(field: string, difficulty: string, questionCount: number): MockInterview {
    logger.warn(`Returning fallback interview for ${field}`);
  ...
  private getFallbackFeedback(
    ...
    // Key points coverage
    score += Math.min(20, keyPointsMentioned.length * 10);
    
    // Variety of words (not repeating same words)
    const uniqueWords = new Set(words.map(w => w.toLowerCase()));
    if (uniqueWords.size / wordCount > 0.7) score += 10;
    ...
    if (keyPointsMentioned.length > 0) strengths.push(`You mentioned key concepts: ${keyPointsMentioned.slice(0, 2).join(', ')}`);
    if (uniqueWords.size / wordCount > 0.7) strengths.push('Good vocabulary variety in your answer');
    if (strengths.length === 0) strengths.push('You attempted to answer the question');
    
    // Generate dynamic improvements
    const improvements: string[] = [];
    if (wordCount < 30) improvements.push('Try to elaborate more on your points (aim for 50-150 words)');
    if (keyPointsMentioned.length < (expectedKeyPoints?.length || 0)) {
      const missed = expectedKeyPoints?.filter(p => !keyPointsMentioned.includes(p)).slice(0, 2) || [];
      if (missed.length > 0) improvements.push(`Consider discussing: ${missed.join(', ')}`);
    }
    if (wordCount < 50) improvements.push('Include specific examples from your experience');
    if (!userAnswer?.match(/\b(I|my|we|our)\b/i)) improvements.push('Use personal examples to make your answer more engaging');
    if (improvements.length === 0) improvements.push('Structure your answer using the STAR method (Situation, Task, Action, Result)');
    
    // Generate contextual detailed feedback
    let detailedFeedback = `Your answer to this ${field || 'interview'} question `;
    if (score >= 70) {
      detailedFeedback += 'demonstrates good understanding. ';
    } else if (score >= 50) {
      detailedFeedback += 'shows potential but needs development. ';
    } else {
      detailedFeedback += 'needs significant improvement. ';
    }
    
    if (keyPointsMentioned.length > 0) {
      detailedFeedback += `You successfully covered ${keyPointsMentioned.length} key point(s). `;
    }
    
    detailedFeedback += 'To strengthen your response, focus on providing concrete examples and structuring your thoughts clearly.';
    ...
    return {
      ...
      detailedFeedback,
      keyPointsCovered: keyPointsMentioned,
      keyPointsMissed: expectedKeyPoints?.filter(p => !keyPointsMentioned.includes(p)) || [],
      suggestedAnswer: `A strong answer for this ${field || 'role'} would include: specific examples demonstrating your experience, clear explanation of your approach, and quantifiable results where possible.`,
    };
  }
  ...
  clearCache(field?: string): void {
    if (field) {
      ...
      logger.info(`Cleared cache for field: ${field}`);
    } else {
      this.cache.clear();
      logger.info('Cleared entire mock interview cache');
    }
  }
}
