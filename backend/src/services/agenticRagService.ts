...
export class AgenticRagService {
  private static instance: AgenticRagService;
   ...
   */
  async classifyMessage(message: string, videoTitle: string): Promise<MessageClassification> {
    const startTime = Date.now();
    
    // Quick pattern check for obvious cases (saves API call)
    const quickCheck = this.quickPatternCheck(message);
    if (quickCheck) {
      logger.info(`Quick pattern classification: ${quickCheck.category} in ${Date.now() - startTime}ms`);
      return quickCheck;
    }
...
Respond with ONLY ONE WORD: EDUCATIONAL, GREETING, OFF_TOPIC, or INAPPROPRIATE`;
      ...
      logger.info(`LLM message classification: ${response} in ${elapsed}ms`);
      ...
      logger.warn(`Message classification failed, defaulting to educational:`, error);
   ...
   */
  private quickPatternCheck(message: string): MessageClassification | null {
  ...
  async analyzeIntent(question: string, videoTitle?: string): Promise<IntentAnalysis> {
    ...
    const useLLM =
      String(process.env.AGENTIC_INTENT_USE_LLM || 'false').toLowerCase() === 'true';

    if (!useLLM || rulesBasedIntent.confidence >= 0.75) {
      logger.info(
        `Rules-based intent: ${rulesBasedIntent.intent} (${Math.round(
          rulesBasedIntent.confidence * 100
        )}%)`,
      );
    ...
    logger.info(
      `LLM intent analysis: ${llmIntent.intent}, retrieval: ${llmIntent.requiresRetrieval}, ${elapsed}ms`,
    );

    return llmIntent;
  }

  private rulesBasedClassification(question: string): IntentAnalysis {
  ...
  private async llmBasedClassification(question: string, videoTitle?: string): Promise<IntentAnalysis> {
    try {
      // Add timeout to prevent hanging
      const timeoutPromise = new Promise<never>((_, reject) => 
        setTimeout(() => reject(new Error('Intent classification timeout')), 5000)
      );
  ...
  private async performLLMClassification(question: string, videoTitle?: string): Promise<IntentAnalysis> {
    try {
...
Classify this question's intent.`;
      ...
      try {
        ...
        logger.warn('Failed to parse LLM intent response, using fallback:', response.answer?.slice(0, 100));
...
export const agenticRagService = AgenticRagService.getInstance();
